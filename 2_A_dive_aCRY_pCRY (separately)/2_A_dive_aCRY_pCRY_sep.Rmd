---
title: "RNAseq aCRY pCRY data dive (separate)"
author: "Kelterborn"
date: "2024-04-19"
output:
  github_document:
...
```{r f_topgenes}
topgenes_f <- function(res,p=0.5,bM=20,l2FC=1){
...
# define list 1
if(is.null(list1) ) {
  list1 <- topgenes_f(res,p=0.05,bM = 10,l2FC = 1)
}
# define list 2
if(is.null(list2) ) {
  list2 <- topgenes_f(res,p=0.005,bM = 100,l2FC = 2)
}
# list 1 color
...
# add symbol
res$symbol <- mcols(dds_list[[i]])$symbol

# define list groups
res$isl1 <- rownames(res) %in% rownames(list1)
res$isl2 <- rownames(res) %in% rownames(list2)

# order results (list 1 on top)
...
ev_f <- EnhancedVolcano(res,
    ...
    y = 'padj',
    lab = res$symbol,
    selectLab  = res$symbol[which(names(keyvals) == list2.n)],
    labSize = 3,
    drawConnectors = TRUE,
    boxedLabels = FALSE,
    widthConnectors = 0.5,
    ...
    ylab = "Padj (-log10)",
    title = n,
    subtitle = paste("list1:",nrow(list1),", list2: ",nrow(list2)),
    # sub = "SVF",
    # pCutoff = 10^(-60),
    # FCcutoff = 1,
    shapeCustom =shape,
...
# 1. Load aCRY & pCRY
## - Tximeta files
```{r get_tximeta_files, include=FALSE}
...
par(mfrow=c(1,2))
hist(log(counts(dds)), breaks=100, ylim = c(0,11000), xlim = c(0,10), main = colData(dds)$experiment[1]) 
...
plotDispEsts(dds, main = colData(dds)$experiment[1])
resultsNames(dds)
DESeq2::plotMA(dds, main=colData(dds)$experiment[1])

ROC40 <- "Cre06.g275350"
plotCounts(dds, gene = ROC40, intgroup = "condition", col=colData(dds)$treatment, main = paste("ROC40 in Δ",colData(dds)$experiment[1],sep = ""))
...
vsd_list <- lapply(dds_list,vst,blind=FALSE)
ntd_list <- lapply(dds_list,normTransform)
rld_list <- lapply(dds_list,rlog, blind=FALSE)
# save(rld_list, file = paste(dir,"rlog_list.RDS",sep="/"), compress = FALSE)

```

```{r pre_trans_fig, figures-side, fig.show="hold", out.width="50%"}
...
```{r pre_sample_dist, fig.height=4, out.width="100%", eval=TRUE, include=TRUE, echo=FALSE}
vsd <- vsd_list[[1]]
for (vsd in vsd_list){
...
```{r pca, warning = FALSE, fig.show="hold", out.width="80%", fig.height=5, eval=TRUE, include=TRUE, echo=FALSE}
# transform data
...
g3 <- ggplot(pcaData, aes(PC1, PC2, color=condition, shape=genotype, label=clientName)) +
  geom_point(size=5, alpha=0.7) +
  geom_text_repel() +
...
gp <- ((g1+g2) / (g3)) + plot_layout(guides = "collect", axis_titles="collect") & plot_annotation(title = colData(vsd)$experiment[1])
gp %>% print()
}
...
p1 <- pairsplot(p,colby = 'treatment', colkey = anno_colors$treatment,title = 'treatment',titleLabSize = 8,trianglelabSize = 6,
...
p2 <- pairsplot(p,colby = 'genotype', colkey = anno_colors$genotype,title = 'genotype',titleLabSize = 8,trianglelabSize = 6,
                hline = 0, vline = 0,gridlines.major = FALSE, gridlines.minor = FALSE,
                plotaxes = FALSE,margingaps = unit(c(-0.01, -0.01, -0.01, -0.01), 'cm')) # -> PC1&3 = genotype

p4 <- pairsplot(p,colby = 'condition', colkey = viridis(length(levels(colData(dds)$condition))),title = 'condition',titleLabSize = 8,trianglelabSize = 6,
                hline = 0, vline = 0,gridlines.major = FALSE, gridlines.minor = FALSE,
                plotaxes = FALSE,margingaps = unit(c(-0.01, -0.01, -0.01, -0.01), 'cm')) # PC1 & PC3 = condition

pairs <- (p1+p2) / (p4) + plot_annotation(title = colData(dds)$experiment[1])
pairs %>% print()
...
s <- screeplot(p,
    ...
    geom_label(aes(x = elbow + 1, y = 50,
      label = paste('Elbow',elbow,sep="="), vjust = -1, size = 8)) + plot_annotation(title = colData(dds)$experiment[1])

s %>% print()

bi <- biplot(p,x="PC3",y="PC1",
    lab = p$metadata$clientName,
...
pairs_bi <- pairs + bi + 
  plot_layout(design = layout) & plot_annotation(title = colData(dds)$experiment[1])
pairs_bi %>% print()
...
## Plot Counts
```{r plot_counts, echo=FALSE, fig.width=12}
dds <- dds_list[[1]]
for (dds in dds_list){
# TF genes
...
gcounts <- ggplot(d, aes(x = treatment, y = count, fill=condition, color=condition)) +
  ...
  geom_text_repel(aes(label=clientName)) +
    ...
    labs(title = paste("ROC40"," in Δ",colData(dds)$experiment[1],sep = ""))
gcounts %>% print()
...
#      [,1]                         [,2]
# [1,] "Intercept"                  "0" 
# [2,] "genotype_pcry_vs_WT"        "0" 
# [3,] "treatment_blue_vs_dark"     "0" 
# [4,] "treatment_red_vs_dark"      "0" 
# [5,] "genotypepcry.treatmentblue" "0" 
# [6,] "genotypepcry.treatmentred"  "0" 

# assign(paste("res",n,sep="_"),list())

res_list <- list()
for (i in 1:2) {
dds <- dds_list[[i]]

nlist <- names(dds_list[i])
c <- resultsNames(dds)
print(nlist)
res_l <- list()
...
# Belichtungseffect für jeden Genotyp
nres <- "WT_BL.vs.D"
contrast <- list(c("treatment_blue_vs_dark"))
res_l[nres] <- getresults_SK(contrast = contrast, nres = nres)

nres <- "WT_R.vs.D"
contrast <- list(c("treatment_red_vs_dark"))
res_l[nres] <- getresults_SK(contrast = contrast, nres = nres)

nres <- paste(nlist,"_BL.vs.D",sep="")
contrast <- list(c("treatment_blue_vs_dark",c[5])) # [5,] "genotypepcry.treatmentblue"
res_l[nres] <- getresults_SK(contrast = contrast, nres = nres)

nres <- paste(nlist,"_R.vs.D",sep="")
contrast <- list(c("treatment_red_vs_dark",c[6])) # [6,] "genotypepcry.treatmentred"  
res_l[nres] <- getresults_SK(contrast = contrast, nres = nres)


# Unterschiede zwischen WT und ko unter gleicher Belichtung
nres <- paste(nlist,"D.vs.WT_D",sep="_")
contrast <- list(c[2])                # [2,] "genotype_pcry_vs_WT"
res_l[nres] <- getresults_SK(contrast = contrast, nres = nres)

nres <- paste(nlist,"BL.vs.WT_BL",sep="_")
contrast <- list(c[2],c[5])           # [5,] "genotypepcry.treatmentblue"
res_l[nres] <- getresults_SK(contrast = contrast, nres = nres)

nres <- paste(nlist,"R.vs.WT_R",sep="_")
contrast <- list(c[2],c[6])           # [6,] "genotypepcry.treatmentred"  
res_l[nres] <- getresults_SK(contrast = contrast, nres = nres)


# Unterschiede im Belichtungseffekt
nres <- paste(nlist,"BLvD.vs.WT_BLvD",sep="_")
contrast <- c(0,0,0,0,1,0)
res_l[nres] <- getresults_SK(contrast = contrast, nres = nres)

nres <- paste(nlist,"RvD.vs.WT_RvD",sep="_")
...
nres <- "BL+R.vs.D"
contrast <- c(0,0,2/6,2/6,1/3,1/3)
res_l[nres] <- getresults_SK(contrast = contrast, nres = nres)

nres <- "BL.vs.D"
contrast <- c(0,0,1,0,0.5,0)
res_l[nres] <- getresults_SK(contrast = contrast, nres = nres)

nres <- "R.vs.D"
contrast <- c(0,0,0,1,0,0.5)
res_l[nres] <- getresults_SK(contrast = contrast, nres = nres)

# WT vs. ko
nres <- paste(nlist,".vs.WT",sep="")
contrast <- c(0,1,0,0,1/3,1/3)
res_l[nres] <- getresults_SK(contrast = contrast, nres = nres)
...
deg_genes_l <- list()
deg_genes_l <- lapply(res_list[[i]],topgenes_f) %>%  lapply(.,rownames) 
names(deg_genes_l) <- paste("deg",names(res_list[[i]]),sep="_")
deg_list[[ko]] <- deg_genes_l

top_genes_l <- list()
top_genes_l <- lapply(res_list[[i]],topgenes_f,p=0.01, bM=100, l2FC=2) %>%  lapply(.,rownames) 
names(top_genes_l) <- paste("top",names(res_list[[i]]),sep="_")
top_list[[ko]] <- top_genes_l

}

top_genes_l <- lapply(res_list,topgenes_f,p=0.01, bM=100, l2FC=2) %>%  lapply(.,rownames) 
...
l <- length(res_list[[i]])
for (j in 1:l){
res_ashr_l[[names(res_list[[i]])[j]]] <- lfcShrink(res=res_list[[i]][[j]], dds=dds_list[[i]], type="ashr")
}
res_ashr_list[[ko]] <- res_ashr_l
}
...
Vulcano_SK(res_ashr_list[[i]]["pcry_BL.vs.WT_BL"],xlim=5,ylim=50)
