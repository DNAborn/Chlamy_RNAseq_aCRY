...
library(DESeq2) # Main RNA-Seq Package
...
# Server
server <- "/Volumes/biopro3/ExpBP/Chlamyland/RNA_seq"
list.files(server)

# for test folder
dir <- "/Volumes/biopro3/ExpBP/Chlamyland/RNA_seq/test" # Working directory
quantdir <- "/Volumes/biopro3/ExpBP/Chlamyland/RNA_seq/test/quants" # quant is the output of the mapping
figuredir <- "/Volumes/biopro3/ExpBP/Chlamyland/RNA_seq/test/figures"


```

## -Connect to Linux machine (Enno))
```{r, eval=FALSE}
# run in terminal (mac)
# run in cmd.exe (Windows)

ssh -Y student@141.20.64.235
# Password: StudentExpBP

# activate salmon
conda activate /home/student/conda_salmon

# select server
cd /ExpBP/Chlamyland/RNA_seq
cd test
...
## -Download via command line in linux
...
salmon index -t gentrome_chlamy.v6.1.fa.gz -i Chlamy_index_Olga  -k 31 -p 10

# --decoys decoys_chlamy_v6.1.txt

```



### 1c. Mapping / Quantifying
...
# test folder
quantdir="/ExpBP/Chlamyland/RNA_seq/test/quants"
indexdir="/ExpBP/Chlamyland/RNA_seq/test/Chlamy_index_Olga"
fasta="/ExpBP/Chlamyland/RNA_seq/test/fastq"

# "Real" data Chlamy_RNASeq_aCRY
quantdir="/ExpBP/Chlamyland/RNA_seq/Chlamy_RNASeq_aCRY/quants_v2"
indexdir="/ExpBP/Chlamyland/RNA_seq/test/Chlamy_index_Olga"
fasta="/ExpBP/Chlamyland/RNA_seq/Chlamy_RNASeq_aCRY/fastq"

# go to fastq folder
fastqfiles=$(\ls *_1.fq.gz)
...
salmon quant -i $indexdir -l A \
  -1 $R1 \
  -2 $R2 \
  -p 15 --validateMappings --gcBias -o $outquant
...
# condition
Samplepath <- filePath(path="/Users/olga/Desktop/RNA-Seq Simon/Chlamy/Server/aCRY/", file="sample_table.xlsx")
...
m.table <- data.frame(sample.table,mappingrates) # here used earlier only for columns [1:2,], otherwise for the whole table use sample.table
```
...
xx <- barplot(height=mappingrates, cex.names=0.5, names=samplename[2], horiz=T, las=1, xlim=c(0,100))
...
## 2c. Tximeta
combine counts from different transcripts (splice variants) to gene level
...
files={}
for (i in list.dirs(path = quantdir, full.names = TRUE, recursive = FALSE)) {
...
coldata <- data.frame(coldata,sample.table[1:2,])
coldata %>% head() %>% kable()
...
indexDir="/Volumes/biopro3/ExpBP/Chlamyland/RNA_seq/test/Chlamy_index_Olga" # salmon index
fastaPath="/Volumes/biopro3/ExpBP/Chlamyland/RNA_seq/salmon_index_chlamy/Phytozone_v6.1/CreinhardtiiCC_4532_707_v6.0.hardmasked.fa.gz" # genomic fasta
gtfPath="/Volumes/biopro3/ExpBP/Chlamyland/RNA_seq/salmon_index_chlamy/Phytozone_v6.1/CreinhardtiiCC_4532_707_v6.1.gene.gff3.gz"
...
# Column infos = Sample infos -> colData(se)
colData(se) %>% head() %>% kable()

# Row Infos = Gene Infos -> mcols(se)
mcols(se) %>% head() %>% kable()
...
TXNAME <- as.character(mapIds(edb,keys = mcols(gse)$gene_id, keytype = "GENEID", column = "TXNAME", multiVals="first"))
...
getwd()
save(gse,file=paste(dir,"tximeta_test.RDS", sep="/"))
# gse <- 1
...
annopath <- file.path("/Volumes/biopro3/ExpBP/Chlamyland/RNA_seq/salmon_index_chlamy/Phytozone_v6.1/CreinhardtiiCC_4532_707_v6.1.master_annotation_table.tsv")
...
par(mfrow=c(1,2))
hist(log(counts(dds)), breaks=100)
hist(counts(dds), breaks=1000000)
